# Design Document: Finding Strategies That Game the Generalization Test

## 1. Purpose

This document is the method contract for **Finding Strategies That Game the Generalization Test**. It specifies what is
...
One-liner: Search for training strategies that look generalizing under holdout but fail under stricter probes.

Hypothesis: Search for training strategies that look generalizing under holdout but fail under stricter probes.
...
|---|---|---|
| Config schema | `genhack.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `genhack.configs.loader` | Hydra compose API |
| Device | `genhack.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `genhack.models.registry` | Pinned revisions + hardware fit |
| Hooks | `genhack.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `genhack.models.generation` | Chat template path recording |
| Data | `genhack.data` | Synthetic items, splits, manifests |
| Evaluation | `genhack.evaluation` | Metrics, failure harness, protocols |
| Ablation | `genhack.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `genhack.reporting` | Aggregate → MD/TeX/figures |
| Cache | `genhack.cache` | Versioned artifact store |
...
`src/genhack/genhack/` are preserved across regeneration.
